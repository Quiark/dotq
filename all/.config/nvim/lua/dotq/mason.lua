...
function M.config()
  M.builtin.mason = {
    ...
    install_root_dir = qutils.join_paths(vim.fn.stdpath "data", "mason"),
...
function M.get_prefix()
  local default_prefix = qutils.join_paths(vim.fn.stdpath "data", "mason")
  return vim.tbl_get(M.builtin, "mason", "install_root_dir") or default_prefix
end

---@param append boolean|nil whether to append to prepend to PATH
local function add_to_path(append)
  local p = qutils.join_paths(M.get_prefix(), "bin")
...
function M.setup()
  ...
  add_to_path(M.builtin.mason.PATH == "append")

  mason.setup(M.builtin.mason)

  if M.builtin.mason.on_config_done then
    M.builtin.mason.on_config_done(mason)
  end
end
